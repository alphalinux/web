...
<?php
	...
	if ($headline != "")
		...
		$query=$query."OR newstxt LIKE '%$newstxt%'";
	...
	function print_results($rec) {
		echo "<TR>";
		echo "<TD>$rec->hdline</TD>";
		echo "<TD>$rec->insdate</TD>";
		if ($rec->shown == 1)
			echo "<TD><FONT COLOR=#00AA00>Yes</FONT></TD>";
		else
			echo "<TD><FONT COLOR=#B70000>No</FONT></TD>";	
		echo "<TD>$rec->catagory</TD>";	
		echo "<TD><A HREF=\"review.php?editor=$rec->indx\">[ Edit ]</TD>";
		echo "</TR>";
	}
      ...
      <TD BGCOLOR="#DC9D33">
		...
		<?php
			if ($num_hits == "")
				...
				echo "<TH ALIGN=LEFT>Headlne</TH><TH ALIGN=LEFT>Date Submitted</TH><TH ALIGN=LEFT>Approved</TH ALIGN=LEFT><TH>Catagory</TH><TH></TH>";
